%Implements Sarsa
function Q = sarsa_main_2(m,n,s,terrain,gamma,episodes,epsilon,g_reward,a_reward,init_q,weights,pun)
...
for j=1:episodes %episodes
    ...
    while (cur_state ~= goal)
        ...
        reward = R(cur_state,a);
        next_state = a;       
        
        actions = find(R(next_state,:)>-inf);
        ...
        a_prime = actions(r(1));
        Q(cur_state,a) = Q(cur_state,a) + (reward + gamma * Q(next_state,a_prime) - Q(cur_state,a)) - (punishment(cur_state, terrain, weights,pun));%punishment(next_state, terrain, weights,pun));
